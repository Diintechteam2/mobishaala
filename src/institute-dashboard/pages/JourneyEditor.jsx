...
const JourneyEditor = () => {
  ...
  const fetchContent = async () => {
    try {
      ...
      if (data.success && data.data.journey) {
        setContent({
          title: data.data.journey.title || '',
          subtitle: data.data.journey.subtitle || '',
          modules: data.data.journey.modules || []
        });
      }
    } catch (err) {
  ...
  const addModule = () => {
    setContent(prev => ({
      ...prev,
      modules: [...prev.modules, { heading: '', detail: '' }]
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://mobishaala-backend-zcxm.onrender.com/api/institute-content/${instituteId}`, {
        ...
        body: JSON.stringify({ journey: content })
      });

      const data = await response.json();
      if (data.success) {
        setSuccess('Journey section updated successfully!');
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Edit Journey/Platform Section</h1>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateModule(index, 'heading', e.target.value)}
                    ...
                    <div>
                      ...
                      <textarea
                        value={module.detail}
                        onChange={(e) => updateModule(index, 'detail', e.target.value)}
                        rows={3}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                      />
                    </div>
